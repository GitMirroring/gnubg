...
PyToPosInfo(PyObject * p, posinfo * ppi)
{
    ...
    while (PyDict_Next(p, &iPos, &pyKey, &pyValue)) {
        ...
        case 3:
            ...
            *((gamestate *) apv[iKey]) = (gamestate) PyLong_AsLong(pyValue);
            break;
...
PythonMatch(PyObject * UNUSED(self), PyObject * args, PyObject * keywds)
{
    ...
    if (g->i != 0) {
        PyErr_SetString(PyExc_Exception, _("First game missing from match"));
        return NULL;
    }
...
PythonRun(const char *sz)
{
    ...
    if (sz && *sz) {
        ...
        if (fX && (py_ret = PyRun_String("gnubg_InteractivePyShell_gui()",
            ...
            if (PyLong_Check(py_ret) && PyLong_AsLong(py_ret))
                success = TRUE;
        if (py_ret) {
            Py_DECREF(py_ret);
        }
